...
)
{
    ...
    try
    {
        ...
        size_t separator;
        if ((separator = uriData.find('/')) == std::string::npos)
        {
            separator = (size_t)-1;
        }
        else
        ...
        std::wstring hostName(LIMPET_STRING_SIZE, L'\0');
        SafeInt<int> hostNameSize = 0;
        hostNameSize = hostName.capacity();
        if (MultiByteToWideChar(CP_UTF8, 0, &uriData[0], -1, &hostName[0], hostNameSize) == 0)
        {
            throw TPM_RC_FAILURE;
        }
        hostName.resize(hostName.find(L'\0'));

        std::wstring deviceId(LIMPET_STRING_SIZE, L'\0');
        if (separator != (size_t)-1)
        {
            // Device name is specified
            SafeInt<int> deviceIdSize = 0;
            deviceIdSize = deviceId.capacity();
            if (MultiByteToWideChar(CP_UTF8, 0, &uriData[separator + 1], -1, &deviceId[0], deviceIdSize) == 0)
            {
                throw TPM_RC_FAILURE;
            }
        }
        ...
        {
            // No device name is specified, use the TPM generated one
            if ((deviceId.capacity() < LIMPET_DEVICE_ID_STRING_LENGTH) ||
                ((result = LimpetReadDeviceId(logicalDeviceNumber, &deviceId[0])) != TPM_RC_SUCCESS))
            {
                throw result;
            }
        }
        deviceId.resize(deviceId.find(L'\0'));
        ...
        return StringCchPrintfW(pszConnectionString,
            *pcchConnectionString,
            L"HostName=%s;DeviceId=%s;SharedAccessSignature=SharedAccessSignature sr=%s/devices/%s&sig=%s&se=%lld",
            &hostName[0],
            &deviceId[0],
            &hostName[0],
            &deviceId[0],
